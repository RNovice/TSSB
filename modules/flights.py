...
def run(API_KEY: str):
  ...
  try:
    ...
    if totalAmount < 100 or (cancelledAmount is not None and cancelledAmount > 30):
      result.update({
        "status": "🚨",
        "header": "🚨",
        "content": f"航班數量過低或取消數量過多。總計{totalAmount}班，{cancelledContent} \n[資料來源](<{sourceUrl}>)"
      })
    elif totalAmount < 200 or (cancelledAmount is not None and cancelledAmount > 15):
      result.update({
        "status": "⚠️",
        "header": "⚠️",
      ...
      result.update({
        "content": f"一切正常。總計{totalAmount}班，{cancelledContent} \n[資料來源](<{sourceUrl}>)"
      })
